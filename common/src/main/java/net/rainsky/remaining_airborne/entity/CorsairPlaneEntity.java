...
public class CorsairPlaneEntity extends AirplaneEntity
{
    public CorsairPlaneEntity(EntityType<? extends AircraftEntity> entityType, Level world) {
        super(entityType, world, true);
    }

    @Override
    public Item asItem() {
        return RemainingAirborne.CORSAIR_PLANE_ITEM.get();
    }
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            if (isWithinParticleRange()) {
                ...
                // Trails
                trail(transform, 0, -7.18f, 2.18f, 0.81f);
                trail(transform, 1, 7.18f, 2.18f, 0.81f);

                // Smoke
                float power = getEnginePower();
                if (power > 0.05) {
                    Vector4f p = transformPosition(transform, (tickCount % 2 == 0 ? -0.36f : 0.36f), 1.51f, 2.37f);
